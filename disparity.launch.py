...
def generate_launch_description():
    ...
    return launch.LaunchDescription([
        ...
        launch_ros.actions.Node(
            ...
            namespace='/right',
            parameters=[{'index': 0, 'fps': 10, 'width': 320, 'height': 240, 'camera_info_path': 'camera-info-right.ini'}],
        ...
        launch_ros.actions.Node(
            package='opencv_cam', executable='opencv_cam_main',
            namespace='/left',
            parameters=[{'index': 2, 'fps':10, 'width': 320, 'height': 240, 'camera_info_path': 'camera-info-left.ini', 'camera_frame_id' : 'test_frame_id'}],
        ...
        launch_ros.actions.Node(
            package='my_vision_package',
            executable='stereo_cluster_wls_node',
            output='screen'
        ),

        # Second disparity node to generate dispR
        launch_ros.actions.Node(
            package='stereo_image_proc',
            executable='disparity_node',
            name='disparity_node_right',
            parameters=[{
                'min_disparity': 0,
                'max_disparity': 96,
                'correlation_window_size': 15,
                'disable_left_right_check': True,
            }],
            remappings=[
                ('left/image_rect', '/right/image_rect_color'),
                ('right/image_rect', '/left/image_rect_color'),
